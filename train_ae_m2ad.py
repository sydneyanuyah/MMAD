#!/usr/bin/env python
"""
Train LSTM Autoencoder + M2AD anomaly detector.
"""
import os
import json
import time
...
def main():
    ...
    print("\nTraining LSTM Autoencoder + M2AD")
    ...
    for chan_id in tqdm(sorted(channels.keys()), desc="lstm_ae_m2ad"):
    ...
    exact_df.to_csv(output_dir / "ae_m2ad_exact_metrics.csv", index=False)
    event_df.to_csv(output_dir / "ae_m2ad_event_metrics.csv", index=False)
    
    with open(output_dir / "ae_m2ad_fit_params.json", "w") as f:
